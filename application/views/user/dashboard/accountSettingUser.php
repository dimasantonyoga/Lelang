...
<body>
                    ...
                    <div class="col-lg-4 col-md-6 px-4 mt-4 d-flex align-items-stretch">
                        <div class="row px-3 " id="album" style="overflow:hidden">
                            <div id="profile" style="border-radius:8px" class="col-lg-12 py-5 text-center bg-white py-3 px-4 shadow w-100 ">
                                ...
                                <h6 id="detailLevel" class="mt-4" style="color: #999">Masyarakat</h6>
                                <h4 id="detailName"><?= $data->nama_lengkap ?></h4>
                                ...
                                <div class="col-lg-12">
                                    <!-- <form action="" id="formEditProfile"> -->
                                        ...
                                        <input type="text" value="<?= $data->nama_lengkap?>"
                                            class="form-control border-success text-success" name="" id="nama_user" aria-describedby="helpId" placeholder="">
                                        <small id="helpId" class="form-text text-muted">Required, max 25 char</small>
                                        </div>
                                        <div class="form-group">
                                        <label for="">Telp</label>
                                        <input type="text" value="<?= $data->telp?>"
                                            class="form-control border-success text-success" name="" id="telp_user" aria-describedby="helpId" placeholder="">
                                        <small id="helpId" class="form-text text-muted">Required, max 25 char</small>
                                        </div>
                                        <div class="form-group">
    ...
    <script>
        ...
        function getData(){
            ...
            $.ajax({
                type: "GET",
                url: "<?= base_url('user/AccountDashboardControllerUser/getData') ?>",
                data: "",
                dataType: "JSON",
                success: function (response) {
                    
                    $("#detailImage").attr("src", base+"assets/img/profile/"+response.foto);
                    $("#detailLevel").html("Masyarakat");
                    $("#detailName").html(response.nama_lengkap);
        ...
        $('#submitImage').submit(function(e){
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url('user/AccountDashboardControllerUser/updateFoto') ?>",
                data: form_data,
                processData: false,
                contentType: false,
                async : true,
        ...
        function validation(){
            // Edit Profile
            // nama_user,username
            if( $('#telp_user').val().length > 0 && $('#telp_user').val().length <= 25 && $('#nama_user').val().length > 0 && $('#nama_user').val().length <= 25 && $('#username').val().length > 0 && $('#username').val().length <= 25){
            ...
            // Border Success
            // Border Name
            if($('#nama_user').val().length > 0 && $('#nama_user').val().length <= 25){
                $('#nama_user').addClass('border-success');
                $('#nama_user').addClass('text-success');
            }else{
                $('#nama_user').removeClass('border-success');
                $('#nama_user').removeClass('text-success');
            }
            // Border Telp
            if($('#telp_user').val().length > 0 && $('#telp_user').val().length <= 25){
                $('#telp_user').addClass('border-success');
                $('#telp_user').addClass('text-success');
            }else{
                $('#telp_user').removeClass('border-success');
                $('#telp_user').removeClass('text-success');
            }
            // Username
        ...
        $("#username").keyup(function (e) {             
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url('user/AccountDashboardControllerUser/usernameCek') ?>",
                data: {
                    "username":username
                },
                dataType: "JSON",
        ...
        $('#buttonEditProfile').click(function() { 
            // DEKLARASI
            let name = $('#nama_user').val();
            let username = $('#username').val();
            let telp = $('#telp_user').val();
            $.ajax({
                type: "POST",
                url: "<?= base_url('user/AccountDashboardControllerUser/updateProfile') ?>",
                data: {
                    'telp' : telp,
                    'nama_lengkap' : name,
                    'username' : username,
        ...
        $("#submitPassword").click(function (e) { 
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url('user/AccountDashboardControllerUser/passwordCek') ?>",
                data: {
                    "password":oldp
                },
                dataType: "JSON",
                success: function (response) {
                    if(response > 0){
                        $.ajax({
                        type: "POST",
                        url: "<?= base_url('user/AccountDashboardControllerUser/passwordUpdate') ?>",
                        data: {
                            "password":newpp
                        },
                        dataType: "JSON",
