...
<body>
    ...
    <script>
        ...
        function getData(fil_number,fil_halaman,fil_status,fil_time,fil_search,fil_kategori){
            ...
            $.ajax({
                type: "GET",
                url: "<?= base_url('user/LelangDashboardControllerUser/getDataDimenangkan/') ?>"+fil_number,
                data: {
                            ...
                            "<div class=' position-absolute shadow' style='top:0;right:0.9rem;left:0.9rem;bottom:0;z-index:1; background:rgba(49, 98, 141, 0.3);display:none;' >"+
                                "<div class='px-5 h-100'>"+
                                    "<div class='h-100 d-flex justify-content-center align-items-center py-2'>"+
                                        "<a data-placement='bottom' title='Detail' style='width:40px;height:40px' class='btn btn-info mx-2 px-1 text-center  rounded-circle' href='<?= base_url(); ?>produk/detail?prdk="+response.result[i].id_barang+"&&nm="+url_set(response.result[i].nama_barang)+"' > <i class='fa fa-book px-2'></i> </a>"+
                                        "<a data-placement='bottom' title='Bayar' style='width:40px;height:40px' class='btn btn-success mx-2 px-1 text-center  rounded-circle' href='<?= base_url(); ?>dashboard/dimenangkan?prdk="+response.result[i].id_barang+"&&nm="+url_set(response.result[i].nama_barang)+"' > <i class='fa fa-dollar-sign px-2'></i> </a>"+
                                    "</div>"+
                                "</div>"+
                            "</div>"+
                            "<div class='row'>"+
